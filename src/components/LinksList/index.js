...
import { Button, List, Card, Icon, Tooltip } from 'antd';
    ...
    return (
      ...
      { selectedMoc && selectedMoc.govtrack_id ? 
        ...
        <List
          ...
          header={<div><strong>{selectedMoc && selectedMoc.member_name ? <div>{selectedMoc.member_name} Links</div> : ''}</strong></div>}
          footer={<div>{selectedMoc && selectedMoc.member_name ? <Tooltip title="Add Link"><Button onClick={() => showModal('add')} type="primary" size="small" style={{ marginLeft: '50%', transform: 'translateX(-50%)' }}><Icon type="plus-circle" /></Button></Tooltip> : ''}</div>}
